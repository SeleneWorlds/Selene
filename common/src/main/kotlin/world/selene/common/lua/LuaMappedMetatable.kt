...
class LuaMappedMetatable(body: (LuaMappedMetatable.() -> Unit)) : LuaMetatable {
    ...
    fun getter(function: KFunction<*>) {
        ...
        require(function.parameters.size == 1) { "Getter '${function.name}' has invalid number of parameters: ${function.parameters.size}" }
    ...
    fun setter(function: KFunction<*>) {
        require(!function.isSuspend) { "Suspend functions are not supported" }
        require(function.parameters.size == 1) { "Setter '${function.name}' has invalid number of parameters: ${function.parameters.size}" }
